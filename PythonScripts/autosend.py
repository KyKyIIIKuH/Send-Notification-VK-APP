...
def senderlistapp():
    ...
    try:
        ...
        cur = con.cursor()
        cur.execute('SET NAMES `utf8`')
        cur.execute("SELECT `id_app`, `message`, `datetime_start`, `uid`, `id`, `useruids`, `category`, `secret_key_app` FROM `vk_app_sender_autosend` WHERE `status`='0' AND `progress`='0' ORDER BY `id` DESC")
        result = cur.fetchall()
        ...
        for row in result:
            ...
            mListUserUIDSField[count] = (row[5])
            mListCategoryField[count] = (row[6])
            mListSecretKeyAppField[count] = (row[7])
            
            fmt = '%Y-%m-%d %H:%M'
            today = datetimenow(fmt, "")
            today_edit = datetimenow(fmt, row[2])
            
            print str(today_edit) + " > " + str(today)
...
def sender():
    ...
    userids_ = mListUserUIDSField[count_selected]
    
    url = "http://ploader.ru/sender/api/load.html"
    method = "POST"
    params = {"action": "sender_message", "auth_key": ""+str(computeMD5hash(str(mListAppID[count_selected])+"_"+str(mListUID[count_selected])+"_"+str(mListSecretKeyAppField[count_selected])))+"", "viewer_id": ""+str(mListUID[count_selected])+"", "app_id": ""+str(mListAppID[count_selected])+"", "userids":""+str(userids_)+"", "category":""+str(mListCategoryField[count_selected])+"", "fromid": ""+str(sCurrent)+""}
    [content, response_code] = fetch_url(url, params, method)
...
def StartFunc():
    ...
    if id_app_db != 0 and message_db != "":
        print str(mListDateTimeBool[count_selected])
        if str(mListDateTimeBool[count_selected]) == "1":
