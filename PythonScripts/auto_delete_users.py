    ...
    try:
        ...
        for row in result:
            ...
            if int(result_day_user_last3) > 40:
                print(str(row[3]) + " last connection " + result_day_user_last + " days")
                delete_user(row[0])
                delete_user_logs(row[2], row[3])
                sleep(1.0)
        
        cur.execute('OPTIMIZE TABLE `vk_app_all_visits`')
        cur.execute('OPTIMIZE TABLE `vk_app_all_visits_logs`')
    except MySQLdb.Error:
        print(db.error())
